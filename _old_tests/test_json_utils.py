...
from ..convert.json_utils import (
    _b64_encode_json,
    _remap_dict_values,
    _get_scale_factors,
...
def test__remap_dict_values():
    ...
    mapping = (("a", "a", lambda x: x + 1), ("b", "y", None), ("c", "z", plus_one))
...
def test__iiif_image_info_json():
    ...
    image_info = _iiif_image_info_json("an_identifier", 3000, 2056)
...
def test_extract_process_kwargs_sizes():
    ...
    expected_kwargs = {"thumbnail_sizes": [1, 2, 3, 4]}
    assert extract_process_kwargs(request) == expected_kwargs
...
def test_add_iiif_info_json():
    response = {"imageId": "an_identifier", "height": 300, "width": 400}
    expected_response = {
