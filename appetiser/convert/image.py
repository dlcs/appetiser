...
def _extract_img_info(img: PILImage) -> dict:
...
def _convert_tiff_mode(
    ...
    with Image.open(image_data, formats=["TIFF"]) as new_img:
        if not new_img.tag_v2.get(x_resolution_tag, None):
            logger.debug(
                "%s: has no XResolution tag, setting from original %s", filepath, x_res
            )
            new_img.tag_v2[x_resolution_tag] = x_res

        if not new_img.tag_v2.get(y_resolution_tag, None):
            logger.debug(
                "%s: has no YResolution tag, setting from original %s", filepath, y_res
            )
...
def _correct_img_orientation(img: PILImage, img_filename: str = "") -> PILImage:
...
def _rasterise_pdf(filepath: Path) -> Path:
    pass
