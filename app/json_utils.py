...
def _get_scale_factors(width: int, height: int, tile_size: int = 256) -> [int]:
    ...
    logger.debug(
        "Calculating scale factors tile_size %s within %s", tile_size, dimension
    )
    factors = [1]
    ...
    logger.debug(
        "Scale factors %s for tile_size %s within %s", factors, tile_size, dimension
    )
    return factors


def _iiif_image_info_json(
    image_id: str, height: int, width: int, tile_size: int = 256
) -> dict:
    ...
    logger.debug(
        "Generating IIIF Image Information JSON for %s (%s, %s) %s",
        image_id,
        width,
        height,
        scale_factors,
...
def add_iiif_info_json(response: dict) -> dict:
    ...
    if all(iiif_image_info_json_args):
        logger.debug(
            "%s: all args present to generate IIIF info json",
