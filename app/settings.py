...
def _get_path_from_env_var(env_var_name: str) -> pathlib.Path:
    ...
    if not env_var:
        raise NameError(f"{env_var_name} is not present as an environment variable")
    path = pathlib.Path(env_var)
    if not path.exists():
        raise FileNotFoundError(f"{env_var} is not an existing file or directory")
...
KDU_LIB, KDU_EXPAND, KDU_COMPRESS = _get_related_env_vars(
    "KDU_LIB", "KDU_COMPRESS", "KDU_EXPAND"
)
