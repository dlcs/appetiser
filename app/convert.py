...
from .jp2 import compress, expand_to_image
...
def _format_paths(
    ...
    if dest:
        ...
        dest_path = OUTPUT_DIR.joinpath(source_path.stem).with_suffix(".jp2")
...
def _format_derivative_info(img: Image, dest_path: pathlib.Path) -> dict:
    ...
    return {"path": str(dest_path), "width": img.width, "height": img.height}


def _make_derivatives(
    source_path: pathlib.Path, derivative_sizes: [int], output_dir: pathlib.Path
) -> list:
    """ Manages the creation of derivatives (only thumbnails at present) for a 
        given JPEG200 file, returning information about the derivatives and where
        they're located.
        """
    ...
    for size in sorted(derivative_sizes, reverse=True):
        ...
        derivative_info.append(_format_derivative_info(img, dest_path))
    return derivative_info


def _ingest_image(
    source_path: pathlib.Path, dest_path: pathlib.Path, optimisation: str
) -> (pathlib.Path, dict):
    ...
    if is_tile_optimised_jp2(prepared_source_path):
        logger.debug("%s: Already a JPEG2000, copying to %s.", source_path, dest_path)
...
def _derivatives_operation(
    ...
    **kwargs
    ...
    if not is_tile_optimised_jp2(source_path):
        return _error("File type must be JPEG2000 for derivatives-only operation.")
    else:
        _, image_info = get_img_info(source_path)
        derivative_info = _make_derivatives(source_path, thumbnail_sizes, thumbnail_dir)
        return {
            "jp2": str(source_path),
...
def _ingest_operation(
    ...
    ingested_path, image_info = _ingest_image(source_path, dest_path, optimisation)
    derivative_info = _make_derivatives(ingested_path, thumbnail_sizes, thumbnail_dir)
    return {
