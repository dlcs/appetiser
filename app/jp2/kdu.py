...
from ..settings import KDU_COMPRESS, KDU_EXPAND, KDU_LIB
...
def kdu_compress(
    ...
    kdu_compress_templates = {
        "kdu_low": "{kdu_compress_path} -i {input_path} -o {output_path} Clevels=7"
        ' "Cblk={{64,64}}" "Cuse_sop=yes" {image_mode} "ORGgen_plt=yes" "ORGtparts=R"'
        ' "Corder=RPCL" -rate 0.5 "Cprecincts={{256,256}},{{256,256}},{{256,256}},'
        '{{128,128}},{{128,128}},{{64,64}},{{64,64}},{{32,32}},{{16,16}}"',
        "kdu_med": "{kdu_compress_path} -i {input_path} -o {output_path} Clevels=7"
        ' "Cblk={{64,64}}" "Cuse_sop=yes" {image_mode} "ORGgen_plt=yes" "ORGtparts=R"'
        ' "Corder=RPCL" -rate 2 "Cprecincts={{256,256}},{{256,256}},{{256,256}},'
        '{{128,128}},{{128,128}},{{64,64}},{{64,64}},{{32,32}},{{16,16}}"',
        "kdu_med_layers": "{kdu_compress_path} -i {input_path} -o {output_path}"
        ' Clevels=7 "Cblk={{64,64}}" "Cuse_sop=yes" {image_mode} "ORGgen_plt=yes"'
        ' "ORGtparts=R" "Corder=RPCL" Clayers=6 -rate 2 "Cprecincts={{256,256}},'
        "{{256,256}},{{256,256}},{{128,128}},{{128,128}},{{64,64}},{{64,64}},"
        '{{32,32}},{{16,16}}"',
        "kdu_high": "{kdu_compress_path} -i {input_path} -o {output_path} Clevels=7"
        ' "Cblk={{64,64}}" "Cuse_sop=yes" {image_mode} "ORGgen_plt=yes" "ORGtparts=R"'
        ' "Corder=RPCL" -rate 4 "Cprecincts={{256,256}},{{256,256}},{{256,256}},'
        '{{128,128}},{{128,128}},{{64,64}},{{64,64}},{{32,32}},{{16,16}}"',
        "kdu_max": "{kdu_compress_path} -i {input_path} -o {output_path} Clevels=7"
        ' "Cblk={{64,64}}" "Cuse_sop=yes" {image_mode} "ORGgen_plt=yes" "ORGtparts=R"'
        ' "Corder=RPCL" -rate - "Cprecincts={{256,256}},{{256,256}},{{256,256}},'
        '{{128,128}},{{128,128}},{{64,64}},{{64,64}},{{32,32}},{{16,16}}"',
    }

    compress_env = {"LD_LIBRARY_PATH": KDU_LIB, "PATH": KDU_COMPRESS}
...
def kdu_expand_to_image(filepath: pathlib.Path) -> Image:
    ...
    expand_env = {"LD_LIBRARY_PATH": KDU_LIB, "PATH": KDU_EXPAND}
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        kdu_expand_command = kdu_expand_template.format(
            kdu_expand_path=KDU_EXPAND, input_path=filepath, output_path=output_file
        )
