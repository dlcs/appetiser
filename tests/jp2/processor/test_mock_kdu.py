...
def test__run_kdu_command(mocker):
    ...
    subprocess.run.assert_called_once_with(
        command, env=env, capture_output=True, check=True, shell=True
    )
...
def test_kdu_compress_kdu_low_rgb(mocker):
    ...
    expected_command = (
        "/path/to/kdu_compress -i /path/to/image.tiff -o /path/to/image.jp2"
    ...
    subprocess.run.assert_called_once_with(
        expected_command, env=expected_env, capture_output=True, check=True, shell=True
    )
...
def test_kdu_compress_kdu_med_rgba(mocker):
    ...
    mocker.patch.dict(
        "os.environ",
        {"KDU_COMPRESS": "/path/to/kdu_compress", "KDU_LIB": "/path/to/kdu_lib",},
    )
    expected_command = (
        "/path/to/kdu_compress -i /path/to/image.tiff -o /path/to/image.jp2"
...
def test_kdu_expand_to_image(mocker):
    ...
    mocker.patch.dict(
        "os.environ",
        {"KDU_EXPAND": "/path/to/kdu_expand", "KDU_LIB": "/path/to/kdu_lib",},
    )
    expected_env = {
        "LD_LIBRARY_PATH": "/path/to/kdu_lib",
